...
class LaporanController extends Controller
{
    ...
    FROM
        (
        SELECT
            ( SELECT sum( a.total ) FROM transaksi a
			JOIN item_penjualan b ON b.no_transaksi = a.no_transaksi 
			JOIN barang c ON b.kode_barang = c.kode_barang 
			JOIN tipe d ON d.kode_tipe = c.kode_tipe WHERE a.tgl_transaksi LIKE '".$tanggal."%' AND a.bpjs IS NULL AND d.jenis_barang = 'barang_lain' ) AS penjualan_barang,
            ( SELECT sum( a.total ) FROM transaksi a
			JOIN item_penjualan b ON b.no_transaksi = a.no_transaksi 
			JOIN barang c ON b.kode_barang = c.kode_barang 
			JOIN tipe d ON d.kode_tipe = c.kode_tipe WHERE a.tgl_transaksi LIKE '".$tanggal."%' AND (a.bpjs IS NULL OR a.bpjs = 0) AND d.jenis_barang = 'obat' ) AS penjualan_obat,
            ( SELECT sum( total ) FROM transaksi WHERE tgl_transaksi LIKE '".$tanggal."%' AND bpjs IS NOT NULL AND bpjs != 0 ) AS piutang_obat,
